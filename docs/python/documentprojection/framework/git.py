...
class GithubPR:
    def __init__(
        ...
        if token is None:
            raise ValueError(
                f"Github access token {token_name} not found in environment variables."
            )
    ...
    def create(self):
        ...
        repo.create_git_ref(
            ...
            sha=repo.get_branch(self.repo_branch_name).commit.sha,
        )

        for file in self.files:
            repo.create_file(
                file[0], "Adding new file", file[1], branch=self.branch_name
            )
        ...
        pull_request = repo.create_pull(
            ...
            base=self.repo_branch_name,
        )
        log.info(
            f"Pull request for {self.repo_name} created successfully: {pull_request.html_url}"
        )
