...
class Tester(ABC):
    ...
    def run_test(self, notebook: Notebook) -> bool:
        log = get_log(self.__class__.__name__)
        log.debug(f"Running tests for {notebook}")
        result = self.test(notebook)
        log.info(f"Tests for {notebook} {'PASSED' if result else 'FAILED'}")
        return result
...
class Channel(ABC):
    ...
    def publish(self, document: Document) -> bool:
        ...
        instance_log.debug(f"Publishing {document} {'SUCCEEDED' if succeeded else 'FAILED'}")
