...
def get_subclasses(module, class_):
    ...
    modules = [importlib.import_module("." + f[:-3], module) for f in files if not f.startswith("__")]
    ...
    return [m for m in all_members if inspect.isclass(m) and issubclass(m, class_) and m != class_]
